...
class FiniteAutomaton {
    ...
    /**
     * Converts the NDFA into a complete DFA using the subset construction algorithm.
     * Each DFA state is represented as a comma-separated string of NDFA states.
     * A trap (dead) state is added to ensure every state has a transition for every symbol.
     */
    convertToDFA() {
        const deadState = "dead"; // trap state label
        ...
        let startSet = new Set([this.initialState]);
        let startStateStr = stateSetToString(startSet);
        dfaStates.add(startStateStr);
        queue.push(startStateStr);
        // Process each subset in the queue.
        while (queue.length > 0) {
            ...
            for (let sym of this.alphabet) {
                ...
                let nextStateStr;
                if (nextSet.size === 0) {
                    nextStateStr = deadState;
                }
                else {
                    nextStateStr = stateSetToString(nextSet);
                }
                dfaTransitions.push({ from: current, symbol: sym, to: nextStateStr });
                // If the state is new and it's not the dead state, add it to the queue.
                if (!dfaStates.has(nextStateStr) && nextStateStr !== deadState) {
                    dfaStates.add(nextStateStr);
                    queue.push(nextStateStr);
                }
            }
        }
        // Add the dead state if it's not already in the set.
        if (!dfaStates.has(deadState)) {
            dfaStates.add(deadState);
        }
        // Define transitions for the dead state (trap state).
        for (let sym of this.alphabet) {
            dfaTransitions.push({ from: deadState, symbol: sym, to: deadState });
        }
